...
def runLSTM():
    ...
    if os.path.exists('model/lstm_model.json'):
        ...
        lstm.add(LSTM(50, return_sequences=True, input_shape=(X_train1.shape[1], X_train1.shape[2])))
...
def graph():
    ...
    if len(mse) < 2:
        messagebox.showerror("Error","Please run ANN and LSTM first")
        return
    ...
    plt.bar(y_pos, mse, color=['blue','green'])
    plt.xticks(y_pos, bars)
    plt.title("ANN vs LSTM Mean Squared Error Comparison")
...
stock_list = ttk.Combobox(main,values=names)
